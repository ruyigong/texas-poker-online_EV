...
HISTORY_MIRROR_ROOTS = [
    APP_ROOT / "hand_history",
    APP_ROOT / "hand_history_online",
]
ROOM_STATE_ROOT = Path(os.environ.get("POKER_STATE_DIR", APP_ROOT / "room_state"))
ROOM_STATE_VERSION = 1
INVITE_CODE = os.environ.get("POKER_INVITE_CODE", "mimidaodao")
...
def history_roots() -> list[Path]:
    roots = [HISTORY_ROOT, *HISTORY_MIRROR_ROOTS]
...
def history_file_path(hand_id: str) -> Path | None:
    ...
    for history_root in history_roots():
        ...
        if path.exists():
            return path
    return (HISTORY_ROOT / hand_id / "betting_history.json").resolve()
...
def list_saved_histories(limit: int = 50) -> list[dict[str, Any]]:
    ...
    for root in history_roots():
        if not root.exists():
            continue
        for path in root.glob("*/betting_history.json"):
...
class Room:
    ...
    worship_enabled: bool = True
    host_player_id: str = ""
    bot_name_counter: int = 0
    removed_scoreboard_rows: list[dict[str, Any]] = field(default_factory=list)
    ...
    stat_recorded_hand_ids: list[str] = field(default_factory=list)
    ...
    def host_id(self) -> str:
        if self.host_player_id in self.players:
            return self.host_player_id
        first = next(iter(self.players), "")
        if first:
            self.host_player_id = first
        return self.host_player_id

    def is_host(self, player_id: str) -> bool:
        return bool(player_id and player_id == self.host_id())

    def require_host(self, player_id: str) -> str | None:
        return None if self.is_host(player_id) else "Only the host can change table settings."

    def bot_count(self) -> int:
        return sum(1 for player in self.players.values() if player.is_bot)

    def removed_scoreboard_names(self) -> set[str]:
        return {str(row.get("name") or "") for row in self.removed_scoreboard_rows}

    def next_bot_name(self) -> str:
        used = {player.name for player in self.players.values()} | self.removed_scoreboard_names()
        while True:
            self.bot_name_counter += 1
            name = f"Bot {self.bot_name_counter}"
            if name not in used:
                return name

    def add_bot(self, player_id: str) -> str | None:
        host_error = self.require_host(player_id)
        if host_error:
            return host_error
        if self.stage not in {"lobby", "showdown"}:
            return "Bots can be added between hands."
        if self.squid_blocks_table_changes():
            return "Bots can join after this squid series ends."
        if self.seated_count() >= MAX_SEATS:
            return "Table is full."
        name = self.next_bot_name()
        bot = Player(id=f"bot-{uuid4()}", name=name, connected=True, ready_next_hand=True, is_bot=True)
        self.players[bot.id] = bot
        self.message = f"{name} joined the table."
        return None

    def remove_bot(self, player_id: str, bot_id: str = "") -> str | None:
        host_error = self.require_host(player_id)
        if host_error:
            return host_error
        if self.stage not in {"lobby", "showdown"}:
            return "Bots can be removed between hands."
        bot_id = bot_id or next((pid for pid, player in reversed(list(self.players.items())) if player.is_bot), "")
        if not bot_id:
            return "There are no bots to remove."
        bot = self.players.get(bot_id)
        if bot is None or not bot.is_bot:
            return "Choose a bot to remove."
        bot_name = bot.name
        bot_row = self.scoreboard_row_for_player(bot)
        self.removed_scoreboard_rows = [row for row in self.removed_scoreboard_rows if row.get("id") != bot.id]
        self.removed_scoreboard_rows.append(bot_row)
        self.removed_scoreboard_rows = self.removed_scoreboard_rows[-50:]
        self.players.pop(bot_id, None)
        self.message = f"{bot_name} removed from the table."
        if self.host_player_id == bot_id:
            self.host_player_id = self.host_id()
        return None
    ...
    def worship_summary(self) -> dict[str, Any]:
        actions = [entry for entry in self.action_log if entry.get("action") == "worship"]
        rows = []
        for entry in actions:
            note = str(entry.get("note") or "")
            target = note[3:] if note.startswith("to ") else note
            rows.append({
                "time": entry.get("time", ""),
                "player": entry.get("player") or "Player",
                "target": target or "Player",
                "amount": int(entry.get("amount") or 0),
                "stackBefore": entry.get("stackBefore"),
                "stackAfter": entry.get("stackAfter"),
            })
        return {"count": len(actions), "total": sum(row["amount"] for row in rows), "actions": rows[-30:]}

    def current_hand_stats(self) -> dict[str, dict[str, bool | int]]:
        stats: dict[str, dict[str, bool | int]] = {}
        if not self.hand_id or self.hand_id in self.stat_recorded_hand_ids:
            return stats
        for player_id in self.hand_start_stacks:
            stats[player_id] = {"hand": True, "vpip": False, "pfr": False}
        for entry in self.action_log:
            player_id = entry.get("playerId")
            action = str(entry.get("action") or "").casefold()
            stage = entry.get("stage")
            if player_id not in stats or (stage != "preflop" and action != "straddle"):
                continue
            amount = int(entry.get("amount") or 0)
            if action in {"call", "raise", "all-in", "call all-in", "straddle"} and amount > 0:
                stats[player_id]["vpip"] = True
            if action == "raise":
                stats[player_id]["pfr"] = True
        return stats

    def record_current_hand_stats(self) -> None:
        if not self.hand_id or self.hand_id in self.stat_recorded_hand_ids:
            return
        stats = self.current_hand_stats()
        for player_id, row in stats.items():
            ...
            if row.get("pfr"):
                player.stat_pfr += 1
        self.stat_recorded_hand_ids.append(self.hand_id)
        self.stat_recorded_hand_ids = self.stat_recorded_hand_ids[-200:]
    def scoreboard_row_for_player(self, player: Player) -> dict[str, Any]:
        ...
        live_stats = self.current_hand_stats().get(player.id, {})
        stat_hands = player.stat_hands + (1 if live_stats.get("hand") else 0)
        stat_vpip = player.stat_vpip + (1 if live_stats.get("vpip") else 0)
        stat_pfr = player.stat_pfr + (1 if live_stats.get("pfr") else 0)
        vpip = round(stat_vpip * 100 / stat_hands) if stat_hands else 0
        pfr = round(stat_pfr * 100 / stat_hands) if stat_hands else 0
        return {"id": player.id, "name": player.name, "chips": player.chips, "buyIns": effective_units, "buyInChips": buy_in, "net": player.chips - buy_in, "vpip": vpip, "pfr": pfr, "statHands": stat_hands, "isPokerGod": False}
    ...
    def scoreboard_rows(self) -> list[dict[str, Any]]:
        ...
        for player in self.ordered_players():
            if not (player.has_played or player.id in played_ids or player.name in played_names or not player.is_spectator or player.rebuy_units != 1 or player.pending_rebuy_units):
                continue
            rows.append(self.scoreboard_row_for_player(player))
            current_ids.add(player.id)
        for row in self.removed_scoreboard_rows:
            if row.get("id") not in current_ids:
                rows.append(row)
        rows.append({"id": "poker-god", "name": "Poker God", "chips": self.poker_god_chips, "buyIns": 0, "buyInChips": 0, "net": self.poker_god_chips, "vpip": None, "pfr": None, "statHands": 0, "isPokerGod": True})
        return rows
    ...
    def worship_player(self, giver_id: str, target_id: str, amount: int) -> str | None:
        giver = self.players[giver_id]
        target = self.players.get(target_id)
        if not self.worship_enabled:
            return "Worship is disabled."
        if giver.is_spectator:
            return "Spectators cannot worship."
        if target is None or target.is_spectator:
            return "Choose a seated player to worship."
        if target.id == giver.id:
            return "You cannot worship yourself."
        if amount <= 0:
            return "Worship amount must be positive."
        if giver.chips < amount:
            return "You do not have enough chips to worship."
        giver_before = giver.chips
        target_before = target.chips
        giver.chips -= amount
        target.chips += amount
        self.log_action(giver, "worship", amount, note=f"to {target.name}", thinking_time=0.0, stack_before=giver_before, stack_after=giver.chips)
        self.log_action(target, "worship receive", amount, note=f"from {giver.name}", thinking_time=0.0, stack_before=target_before, stack_after=target.chips)
        self.message = f"{giver.name} worships {target.name} {amount}."
        return None
    ...
    def hand_history_payload(self, completed: bool = False) -> dict[str, Any]:
        ...
        return {
            ...
            "worshipSummary": self.worship_summary(),
    ...
    def write_hand_history(self, completed: bool = False) -> dict[str, Any] | None:
        ...
        wrote = False
        for root in history_roots():
            folder = root / self.hand_id
            try:
                folder.mkdir(parents=True, exist_ok=True)
                (folder / "betting_history.json").write_text(text, encoding="utf-8")
                wrote = True
            except OSError as exc:
                print(f"Could not write hand history to {folder}: {exc}")
        if not wrote:
            print(f"Hand history was not saved for {self.hand_id}")
        return payload

    def save_hand_history(self) -> None:
        self.record_current_hand_stats()
    ...
    def public_state(self) -> dict[str, Any]:
        ...
        worship_summary = self.worship_summary()
        scoreboard = self.scoreboard_rows()
        ...
        return {
            ...
            "peekEnabled": self.peek_enabled,
            "worshipEnabled": self.worship_enabled,
            "hostPlayerId": self.host_id(),
            "buyTimePrice": self.big_blind * 2,
            "pokerGodChips": self.poker_god_chips,
            "peekSummary": peek_summary,
            "worshipSummary": worship_summary,
            ...
            "squid": {
                ...
                "canConfigure": not self.squid_blocks_table_changes() or self.squid_pending_player_disconnected(),
                "canRestart": self.squid_pending_player_disconnected(),
                "minimum": self.min_squid_total(),
            },
            "canStartHand": self.can_start_hand(),
            "startBlockedReason": self.start_blocked_reason(),
            "maxSeats": MAX_SEATS,
            "players": [
                {
                    "id": player.id,
    ...
    def private_state_for(self, player_id: str) -> dict[str, Any]:
        ...
        return {
            ...
            "you": {
                ...
                "wantsSeat": player.wants_seat,
                "canRequestSeat": player.is_spectator and not self.squid_blocks_table_changes(),
                "canAct": self.can_player_act(player_id),
                "canPrefold": self.can_player_prefold(player_id),
                "prefold": player.prefold,
                ...
                "canShuffleSeats": self.is_host(player_id) and self.stage in {"lobby", "showdown"} and not player.is_spectator,
            },
        }
    ...
    def pending_ready_names(self) -> list[str]:
        return [
            player.name
            for player in self.seated_players()
            if player.connected and not player.is_bot and not player.ready_next_hand
        ]
    ...
    def bot_preflop_strength(self, player: Player) -> float:
        if len(player.cards) != 2:
            return 0.0
        values = sorted((RANK_VALUES[rank_of(card)] for card in player.cards), reverse=True)
        high, low = values
        paired = high == low
        suited = suit_of(player.cards[0]) == suit_of(player.cards[1])
        gap = high - low
        broadways = sum(1 for value in values if value >= 10)
        if paired:
            score = 0.52 + high / 22
        else:
            score = (high / 18) + (low / 32) + (0.08 if suited else 0) + (0.06 if gap <= 1 else 0) + (0.04 if broadways == 2 else 0)
        if {high, low} == {14, 2} and suited:
            score += 0.04
        return max(0.05, min(0.98, score))

    def bot_postflop_strength(self, player: Player, iterations: int = 64) -> float:
        visible = self.visible_community()
        if len(visible) < 3 or len(player.cards) != 2:
            return self.bot_preflop_strength(player)
        known = set(player.cards + visible)
        unknown = [card for card in [f"{rank}{suit}" for suit in SUITS for rank in RANKS] if card not in known]
        opponents = max(1, min(4, len([item for item in self.active_players() if not item.folded and item.id != player.id])))
        wins = 0.0
        trials = max(1, min(iterations, len(unknown)))
        for _ in range(trials):
            ...
            if len(board) < 5:
                continue
            hero_rank = evaluate_best(player.cards + board)
            opponent_ranks = []
            for _opponent in range(opponents):
                if len(deck) < 2:
                    break
                opponent_cards = [deck.pop(), deck.pop()]
                opponent_ranks.append(evaluate_best(opponent_cards + board))
            if not opponent_ranks:
                ...
                if hero_rank > best_opponent:
                    ...
                    wins += 0.5
        return max(0.02, min(0.99, wins / trials))

    def bot_strength(self, player: Player) -> float:
        return self.bot_preflop_strength(player) if self.stage == "preflop" else self.bot_postflop_strength(player)

    def bot_raise_to(self, player: Player, percent: float) -> int | None:
        min_raise_to = self.current_bet + self.min_raise
        if player.current_bet + player.chips <= min_raise_to:
            return None
        bet_size = max(self.min_raise, int(round(self.pot * percent)))
        raise_to = max(min_raise_to, self.current_bet + bet_size)
        return min(raise_to, player.current_bet + player.chips)

    def bot_betting_decision(self, player: Player) -> tuple[str, int]:
        strength = self.bot_strength(player)
        call_amount = max(0, self.current_bet - player.current_bet)
        pot_odds = call_amount / max(1, self.pot + call_amount)
        jitter = random.uniform(-0.08, 0.08)
        if call_amount <= 0:
            if strength + jitter > 0.76:
                raise_to = self.bot_raise_to(player, random.choice([0.4, 0.75]))
                if raise_to:
                    return "raise", raise_to
            if strength + jitter > 0.58 and random.random() < 0.35:
                raise_to = self.bot_raise_to(player, 0.25)
                if raise_to:
                    return "raise", raise_to
            return "check", 0
        if strength + jitter < max(0.22, pot_odds * 1.18):
            return "fold", 0
        if strength + jitter > 0.82:
            raise_to = self.bot_raise_to(player, random.choice([0.4, 0.75]))
            if raise_to:
                return "raise", raise_to
        if strength + jitter > 0.48 or call_amount <= self.big_blind:
            return "call", 0
        return "fold", 0

    def drive_bots(self, limit: int = 40) -> None:
        for _ in range(limit):
            ...
            if self.stage == "straddle":
                player = seated[self.straddle_index]
                if not player.is_bot or not self.can_player_straddle(player.id):
                    return
                should_straddle = self.bot_preflop_strength(player) > 0.82 and random.random() < 0.35
                self.perform_straddle(player.id, should_straddle)
                acted = True
            elif self.stage in BETTING_STAGES:
                player = seated[self.turn_index]
                if not player.is_bot or not self.can_player_act(player.id):
                    return
                action, amount = self.bot_betting_decision(player)
                self.perform_action(player.id, action, amount)
                acted = True
            elif self.stage == "board_choice":
                player_id = self.current_board_choice_player_id()
                player = self.players.get(player_id or "")
                if player is None or not player.is_bot:
                    return
                self.perform_board_choice(player.id, 1)
                acted = True
            elif self.stage == "reveal":
                player_id = self.current_showdown_player_id()
                player = self.players.get(player_id or "")
                if player is None or not player.is_bot:
                    return
                self.perform_showdown_choice(player.id, True)
                acted = True
            elif self.stage == "optional_show":
                player = self.players.get(self.optional_show_player_id or "")
                if player is None or not player.is_bot:
                    return
                self.finish_optional_show(player.id, True)
                acted = True
            else:
                return
            if not acted:
    ...
    def can_player_prefold(self, player_id: str) -> bool:
        player = self.players.get(player_id)
        return bool(
            player is not None
            and self.stage in BETTING_STAGES
            and not player.is_spectator
            and bool(player.cards)
            and not player.folded
            and not player.all_in
            and not self.can_player_act(player_id)
        )

    def toggle_prefold(self, player_id: str) -> str | None:
        player = self.players[player_id]
        if player.prefold:
            player.prefold = False
            return None
        if not self.can_player_prefold(player_id):
            return "Prefold is available before your betting turn."
        player.prefold = True
        return None

    def fold_player(self, player: Player, action: str = "fold", message: str | None = None) -> None:
        player.prefold = False
        player.folded = True
        player.cards_visible = False
        player.checked_marker = False
        player.acted = True
        self.log_action(player, action, 0)
        self.message = message or f"{player.name} folded."

    def apply_prefold_actions(self) -> None:
        guard = 0
        while self.stage in BETTING_STAGES and self.seated_players() and guard < MAX_SEATS:
            guard += 1
            seated = self.seated_players()
            player = seated[self.turn_index]
            if not (player.prefold and player.can_act()):
                break
            self.fold_player(player, "prefold", f"{player.name} folded.")
            live_players = [candidate for candidate in self.active_players() if not candidate.folded]
    ...
    def squid_blocks_table_changes(self) -> bool:
        return self.squid_active and self.squid_pending_player_id is None

    def squid_pending_player_disconnected(self) -> bool:
        if not self.squid_active or not self.squid_pending_player_id:
            return False
        player = self.players.get(self.squid_pending_player_id)
        return player is None or not player.connected

    def configure_squid(self, player_id: str, total: int, price: int) -> str | None:
        host_error = self.require_host(player_id)
        if host_error:
            return host_error
        can_restart_pending = self.squid_pending_player_disconnected()
        if self.squid_blocks_table_changes() and not can_restart_pending:
            return "Active squid settings can be changed after this squid series ends."
        ...
        if self.squid_blocks_table_changes() and total > 0 and total < self.squid_claimed:
        ...
        if total == 0 or price == 0:
            ...
            return None
        if self.squid_active and not can_restart_pending:
    ...
    def can_player_peek(self, viewer_id: str, target_id: str) -> bool:
        ...
        if not self.live_hand_in_progress() or not viewer.folded or viewer.is_spectator or not target.cards:
            return False
        if target.id in set(self.private_card_views.get(viewer.id, [])):
            return False
    ...
    def buy_time(self, player_id: str) -> str | None:
        ...
        return None
    def configure_peek_enabled(self, player_id: str, enabled: bool) -> str | None:
        host_error = self.require_host(player_id)
        if host_error:
            return host_error
        self.peek_enabled = enabled
        state = "enabled" if enabled else "disabled"
        self.message = f"Wanna see see is {state}."
        return None

    def configure_worship_enabled(self, player_id: str, enabled: bool) -> str | None:
        host_error = self.require_host(player_id)
        if host_error:
            return host_error
        self.worship_enabled = enabled
        state = "enabled" if enabled else "disabled"
        self.message = f"Worship is {state}."
        return None

    def configure_blinds(self, player_id: str, small_blind: int, big_blind: int, action_timeout: int | None = None, buy_time_seconds: int | None = None, peek_price_min: int | None = None, peek_price_max: int | None = None) -> str | None:
        host_error = self.require_host(player_id)
        if host_error:
            return host_error
        if self.stage not in {"lobby", "showdown"}:
        ...
        return None
    def start_blocked_reason(self) -> str:
        if self.stage not in {"lobby", "showdown"}:
            return "Hand is already in progress."
        connected = [player for player in self.seated_players() if player.connected]
        seated_with_chips = [
            player
            for player in connected
            if (player.chips + (player.pending_rebuy_units * STARTING_CHIPS if player.refill_submitted else 0)) > 0
        ]
        if len(seated_with_chips) < 2:
            return "Need at least 2 connected seated players\nwith chips or submitted refill to start."
        waiting = [player.name for player in connected if not player.is_bot and not player.ready_next_hand]
        if waiting:
            return "Waiting for ready: " + ", ".join(waiting) + "."
        return ""

    def can_start_hand(self) -> bool:
        return self.start_blocked_reason() == ""
    ...
    def submit_refill(self, player: Player, rebuy_units: int = 0) -> str | None:
        if player.is_spectator:
    ...
    def request_refill(self, player_id: str, rebuy_units: int = 0) -> str | None:
        player = self.players[player_id]
        return self.submit_refill(player, rebuy_units)

    def refill_bot(self, player_id: str, bot_id: str, rebuy_units: int = 0) -> str | None:
        host_error = self.require_host(player_id)
        if host_error:
            return host_error
        bot = self.players.get(bot_id)
        if bot is None or not bot.is_bot:
            return "Choose a bot to refill."
        return self.submit_refill(bot, rebuy_units)
    ...
    def prepare_next_hand_seats(self) -> None:
        for player in self.seated_players():
            if not player.connected:
                ...
                player.pending_rebuy_units = 0
        for player in self.ordered_players():
            if (not self.squid_blocks_table_changes()) and player.is_spectator and player.wants_seat and player.connected and self.seated_count() < MAX_SEATS:
                player.is_spectator = False
                player.wants_seat = False

    def reset_hand(self) -> None:
        if not self.can_start_hand():
            self.message = self.start_blocked_reason() or "Waiting for seated players to click ready."
            return
        ...
        if len(players) < 2:
            self.message = "Need at least 2 seated players\nwith chips to start."
            return
        ...
        self.write_hand_history(completed=False)
    ...
    def perform_timeout(self) -> str | None:
        ...
        if self.stage in BETTING_STAGES:
            ...
            self.fold_player(player, "timeout fold", f"{player.name} timed out and folded.")
            self.after_action()
        return None
...
def player_to_snapshot(player: Player) -> dict[str, Any]:
    ...
    payload["connected"] = bool(player.is_bot)
    return payload
...
def player_from_snapshot(payload: dict[str, Any]) -> Player:
    ...
    values["connected"] = bool(values.get("is_bot"))
...
class RoomManager:
    ...
    def can_delete_room(self, room: Room) -> bool:
        return not room.sockets and not any(player.connected and not player.is_bot for player in room.players.values())
...
async def list_rooms() -> JSONResponse:
    ...
    for room in manager.rooms.values():
        ...
        rooms.append({
            ...
            "canDelete": manager.can_delete_room(room),
        })
...
async def delete_room(room_code: str) -> JSONResponse:
    room = manager.rooms.get(room_code.upper())
    if room is None:
        return JSONResponse({"error": "Room not found"}, status_code=404)
    if not manager.can_delete_room(room):
        return JSONResponse({"error": "Cannot delete a room while players are connected."}, status_code=409)
        ...
        while True:
            ...
            if action == "start_hand":
                ...
                try:
                    ...
                    squid_total = 0
                    squid_price = 0
                error = room.configure_squid(player_id, squid_total, squid_price)
            elif action == "shuffle_seats":
                error = room.shuffle_seats(player_id)
            elif action == "add_bot":
                error = room.add_bot(player_id)
            elif action == "remove_bot":
                error = room.remove_bot(player_id, str(message.get("targetId", "")))
            elif action == "chat":
                ...
                try:
                    ...
                    amount = 0
                error = room.worship_player(player_id, str(message.get("targetId", "")), amount)
                ...
                try:
                    ...
                    peek_price_min = 2
                    peek_price_max = 10
                error = room.configure_blinds(player_id, small_blind, big_blind, action_timeout, buy_time_seconds, peek_price_min, peek_price_max)
            elif action == "set_peek_enabled":
                error = room.configure_peek_enabled(player_id, bool(message.get("enabled")))
            elif action == "set_worship_enabled":
                error = room.configure_worship_enabled(player_id, bool(message.get("enabled")))
            elif action == "prefold":
                error = room.toggle_prefold(player_id)
            ...
            elif action == "refill_bot":
                try:
                    units = int(message.get("units", 0))
                except (TypeError, ValueError):
                    units = 0
                error = room.refill_bot(player_id, str(message.get("targetId", "")), units)
...
const moveStoreKey="pokerPanelPositions:v4";let panelPositions={};try{panelPositions=JSON.parse(localStorage.getItem(moveStoreKey)||"{}")||{}}catch(e){panelPositions={}}let panelZ=140,draggingPanelKey="";function savePanelPositions(){localStorage.setItem(moveStoreKey,JSON.stringify(panelPositions))}function clampPanel(el,left,top){const rect=el.getBoundingClientRect(),pad=8,maxLeft=Math.max(pad,window.innerWidth-rect.width-pad),maxTop=Math.max(pad,window.innerHeight-rect.height-pad);return{left:Math.min(Math.max(pad,left),maxLeft),top:Math.min(Math.max(pad,top),maxTop)}}function applyPanelPosition(el,key){if(draggingPanelKey===key)return;const pos=panelPositions[key];if(!pos)return;el.style.width=pos.width?`${pos.width}px`:el.style.width;el.style.left=`${pos.left}px`;el.style.top=`${pos.top}px`;el.classList.add("drag-floating")}function ensureMoveHandle(el,key,label="Move"){if(!el)return;el.classList.add("movable-panel");applyPanelPosition(el,key);let handle=el.querySelector(":scope > .move-handle");if(!handle){handle=document.createElement("button");handle.type="button";handle.className="move-handle";handle.title=label;handle.setAttribute("aria-label",label);handle.textContent="+";el.appendChild(handle)}handle.onpointerdown=e=>{if(e.button!==undefined&&e.button!==0)return;e.preventDefault();e.stopPropagation();const rect=el.getBoundingClientRect(),startX=e.clientX,startY=e.clientY,startLeft=rect.left,startTop=rect.top,startWidth=rect.width,wasFloating=el.classList.contains("drag-floating");let started=false,placeholder=null;draggingPanelKey=key;handle.setPointerCapture(e.pointerId);const startFloating=()=>{if(started)return;started=true;if(!wasFloating){placeholder=document.createElement("div");placeholder.className="drag-placeholder";placeholder.style.height=`${rect.height}px`;placeholder.style.width=`${rect.width}px`;el.parentNode&&el.parentNode.insertBefore(placeholder,el.nextSibling)}el.style.width=`${startWidth}px`;el.style.left=`${startLeft}px`;el.style.top=`${startTop}px`;el.classList.add("drag-floating");el.style.zIndex=String(++panelZ)};const move=ev=>{const dx=ev.clientX-startX,dy=ev.clientY-startY;if(!started&&Math.hypot(dx,dy)<6)return;startFloating();const next=clampPanel(el,startLeft+dx,startTop+dy);el.style.left=`${next.left}px`;el.style.top=`${next.top}px`};const cleanup=()=>{if(placeholder&&placeholder.parentNode)placeholder.parentNode.removeChild(placeholder);draggingPanelKey=""};const up=ev=>{handle.releasePointerCapture(ev.pointerId);handle.removeEventListener("pointermove",move);handle.removeEventListener("pointerup",up);handle.removeEventListener("pointercancel",up);if(!started){cleanup();return}const finalRect=el.getBoundingClientRect();const next=clampPanel(el,finalRect.left,finalRect.top);panelPositions[key]={left:Math.round(next.left),top:Math.round(next.top),width:Math.round(finalRect.width)};el.style.left=`${next.left}px`;el.style.top=`${next.top}px`;cleanup();savePanelPositions()};handle.addEventListener("pointermove",move);handle.addEventListener("pointerup",up);handle.addEventListener("pointercancel",up)};handle.ondblclick=e=>{e.preventDefault();e.stopPropagation();delete panelPositions[key];savePanelPositions();el.classList.remove("drag-floating");el.style.left="";el.style.top="";el.style.width="";el.style.zIndex=""}}function initMovablePanels(){[[els.pokerGod,"pokerGod"],[els.chatPanel,"chat"],[els.historyPanel,"history"],[$("peekPanel"),"peek"],[$("worshipPanel"),"worship"],[document.querySelector(".actions"),"actions"],[els.rebuyPanel,"refill"],[els.scoreboardPanel,"scoreboard"],[els.squidStatus,"squidStatus"]].forEach(([el,key])=>ensureMoveHandle(el,key))}
function quickBetTo(percent){if(!latest||!latest.state||!latest.you)return;const s=latest.state,you=latest.you,minRaiseTo=Math.max((s.currentBet||0)+(s.minRaise||s.bigBlind),s.bigBlind),betSize=Math.max(s.minRaise||s.bigBlind,Math.round((s.pot||0)*percent)),maxTo=(you.currentBet||0)+(you.chips||0),target=Math.max(minRaiseTo,(s.currentBet||0)+betSize);els.raiseAmount.value=Math.max(0,Math.min(target,maxTo));}
function connect(){if(!roomCode||!playerId)return;if(ws)ws.close();const proto=location.protocol==="https:"?"wss":"ws";ws=new WebSocket(`${proto}://${location.host}/ws/${roomCode}/${playerId}`);ws.onmessage=e=>{try{const p=JSON.parse(e.data);if(p.type==="state"){render(p);initMovablePanels()}if(p.type==="error")els.notice.textContent=p.message}catch(err){console.error(err);els.notice.textContent="Could not render the latest table update. Please refresh."}}}
...
function animateWinnerChips(winners){if(!els.chipFlightLayer||!els.potLabel||!winners.length)return;const shellRect=els.tableShell.getBoundingClientRect(),potRect=els.potLabel.getBoundingClientRect(),sourceX=potRect.left+potRect.width/2-shellRect.left,sourceY=potRect.top+potRect.height/2-shellRect.top;winners.forEach((w,wi)=>{const target=els.seats.querySelector(`[data-player-id="${CSS.escape(w.id)}"] .badge`);if(!target)return;const targetRect=target.getBoundingClientRect(),targetX=targetRect.left+targetRect.width/2-shellRect.left,targetY=targetRect.top+targetRect.height/2-shellRect.top;for(let i=0;i<12;i++){const chip=document.createElement("span");chip.className="flying-chip";chip.style.left=`${sourceX+(i%4-1.5)*6}px`;chip.style.top=`${sourceY+(Math.floor(i/4)-1)*6}px`;chip.style.setProperty("--dx",`${targetX-sourceX+(i%3-1)*16}px`);chip.style.setProperty("--dy",`${targetY-sourceY+(Math.floor(i/3)-1.5)*10}px`);chip.style.animationDelay=`${wi*.16+i*.045}s`;els.chipFlightLayer.appendChild(chip);setTimeout(()=>chip.remove(),3400)}})}
...
function render(payload){latest=payload;const s=payload.state,you=payload.you,seatedRaw=s.players.filter(p=>!p.isSpectator).slice(0,s.maxSeats||10),youSeat=seatedRaw.findIndex(p=>p.id===you.id),seated=youSeat>=0?[...seatedRaw.slice(youSeat),...seatedRaw.slice(0,youSeat)]:seatedRaw;const roomLabel=s.roomName||s.roomCode;els.topbar.textContent=roomCode?`Connected to ${roomLabel}${you.isSpectator?" as spectator":""}`:"";els.meta.textContent=`Room ${roomLabel} | ${s.stage.toUpperCase()} | Blinds ${s.smallBlind}/${s.bigBlind} | Clock ${s.actionTimeout}s | Seats ${seated.length}/${s.maxSeats}`;els.potLabel.textContent=`Total Pot: ${s.pot}`;els.pokerGodChips.textContent=s.pokerGodChips||0;clockDeadline=s.actionDeadline?Math.round(s.actionDeadline*1000):0;updateClock();els.notice.textContent="";els.boardChoiceHint.textContent="";els.boardChoiceHint.classList.add("hidden");const messageParts=splitSquidText(s.message||(you.isSpectator?"Spectating.":""));const playerColorByName=Object.fromEntries((s.players||[]).map(p=>[p.name,playerColors[(p.colorIndex||0)%playerColors.length]]));const boardChoiceActive=!!(s.boardChoice&&s.boardChoice.active);els.tableNotice.innerHTML=boardChoiceActive?"":colorNotice(messageParts.main,playerColorByName);if(!boardChoiceActive&&!s.canStartHand&&s.startBlockedReason&&["lobby","showdown"].includes(s.stage)){els.tableNotice.innerHTML+=(els.tableNotice.innerHTML?"<br>":"")+colorNotice(s.startBlockedReason,playerColorByName)}if(boardChoiceActive&&messageParts.main){els.boardChoiceHint.innerHTML=colorNotice(messageParts.main,playerColorByName);els.boardChoiceHint.classList.remove("hidden");}const history=s.handHistory||[];if(history.length<lastSoundCount)lastSoundCount=0;if(lastSoundCount&&history.length>lastSoundCount){history.slice(lastSoundCount).forEach(h=>{const action=String(h.action||"").toLowerCase();if(action==="check")playSound("check");else if(action==="fold"||action==="timeout fold"||action==="prefold")playSound("fold");else if(/^(small blind|big blind|call|raise|all-in|call all-in|straddle|buy time)$/.test(action))playSound("bet")})}lastSoundCount=history.length;els.historyList.innerHTML=history.length?history.slice().reverse().map(h=>{const color=playerColorByName[h.player]||"#ffe28a",stack=h.stackBefore!==null&&h.stackBefore!==undefined?` <span class="history-stack">(${h.stackBefore} -> ${h.stackAfter})</span>`:"";return `<div class="history-row" style="border-left:4px solid ${color}"><div class="history-main"><b style="color:${color}">${h.player}</b> ${h.action}${h.amount?` ${h.amount}`:""}${stack}</div><div class="history-sub">${String(h.stage).toUpperCase()} | ${h.thinkingTime}s | ${h.time}</div></div>`}).join(""):`<div class="history-empty">No betting yet.</div>`;const peek=s.peekSummary||{},peekRows=peek.actions||[];els.peekSummary.innerHTML=peekRows.length?`<div class="peek-row"><b>Total</b> ${peek.count||0} peek(s), ${peek.total||0} chips to Poker God</div>`+peekRows.slice().reverse().map(p=>`<div class="peek-row"><b>${esc(p.player)}</b> paid ${p.amount}<br>${esc(p.note)}</div>`).join(""):`<div class="history-empty">No peeks yet.</div>`;const worship=s.worshipSummary||{},worshipRows=worship.actions||[];els.worshipSummary.innerHTML=worshipRows.length?`<div class="peek-row"><b>Total</b> ${worship.count||0} worship(s), ${worship.total||0} chips moved</div>`+worshipRows.slice().reverse().map(w=>{const giverColor=playerColorByName[w.player]||"#f7fbff",targetColor=playerColorByName[w.target]||"#f7fbff",stack=w.stackBefore!==null&&w.stackBefore!==undefined?`<br>${w.stackBefore} -> ${w.stackAfter}`:"";return `<div class="peek-row"><b style="color:${giverColor}">${esc(w.player)}</b> -> <b style="color:${targetColor}">${esc(w.target)}</b> ${w.amount}${stack}</div>`}).join(""):`<div class="history-empty">No worship yet.</div>`;const boardRows=s.scoreboard||[];els.scoreboardPanel.innerHTML=`<b>Win / Lose</b><div class="score-table"><div class="score-head">Player</div><div class="score-head">Chips</div><div class="score-head">Buy-ins</div><div class="score-head">Net</div><div class="score-head">VPIP</div><div class="score-head">PFR</div>${boardRows.map(r=>{const rowColor=r.name==="Poker God"?"#ffe28a":playerColorByName[r.name]||"#f7fbff",vpip=r.vpip===null||r.vpip===undefined?"-":`${r.vpip}%`,pfr=r.pfr===null||r.pfr===undefined?"-":`${r.pfr}%`;return `<div style="color:${rowColor}">${esc(r.name)}</div><div>${r.chips}</div><div>${r.buyIns}</div><div class="${r.net>=0?"score-net-pos":"score-net-neg"}">${r.net>=0?"+":""}${r.net}</div><div>${vpip}</div><div>${pfr}</div>`}).join("")}</div>`;const chats=s.chatMessages||[];els.chatMessages.innerHTML=chats.length?chats.map(m=>{const chatColor=playerColorByName[m.player]||"#f7fbff";return `<div class="chat-row"><b style="color:${chatColor}">${esc(m.player)}</b> ${esc(m.text)}</div>`}).join(""):`<div class="chat-row">No messages yet.</div>`;els.chatMessages.scrollTop=els.chatMessages.scrollHeight;const prev=s.previousHands||[];els.previousHands.innerHTML=prev.length?prev.slice().reverse().map(h=>{const stacks=(h.stacks||[]).map(p=>`${esc(p.name)} ${p.before} -> ${p.after}`).join(" | ");return `<div class="previous-hand"><b>${(h.winners||[]).map(w=>`${w.name} +${w.amount}`).join(", ")||"No winner"}</b><br>${h.endedAt}${stacks?`<br>${stacks}`:""}<br>${h.path||""}</div>`}).join(""):"";if(s.bankrollAudit){els.meta.textContent+=` | Bankroll ${s.bankrollAudit.live}/${s.bankrollAudit.effectiveBuyIns}`;}if(s.squid&&s.squid.active){els.meta.textContent+=` | Squids ${s.squid.claimed}/${s.squid.total} @ ${s.squid.price}`;}const squidActive=!!(s.squid&&s.squid.active),squidBlocks=[];if(squidActive){const holdersRaw=(s.squid.holders||[]).map(h=>`${h.name}: ${h.squids}`).join(" | ")||"None yet",holders=colorNotice(holdersRaw,playerColorByName),pending=s.squid.pendingPlayer?`<br>First claim reserved for: ${colorNotice(s.squid.pendingPlayer,playerColorByName)}`:"";squidBlocks.push(`<b>Squid Status</b><br>Claimed: ${s.squid.claimed} | Unclaimed: ${s.squid.remaining} | Price: ${s.squid.price}${pending}<br>Holders: ${holders}`);}if(messageParts.squid){squidBlocks.push(colorNotice(messageParts.squid,playerColorByName));}els.squidStatus.classList.toggle("hidden",!squidBlocks.length);els.squidStatus.innerHTML=squidBlocks.join("<hr>");const hostSettings=!!you.isHost,canConfigureSquid=hostSettings&&!!(s.squid&&s.squid.canConfigure);els.squidTotal.disabled=!canConfigureSquid;els.squidPrice.disabled=!canConfigureSquid;els.squidTotal.min=s.squid&&s.squid.minimum?s.squid.minimum:0;els.peekToggle.textContent=`Wanna See See: ${s.peekEnabled?"On":"Off"}`;els.peekToggle.disabled=!hostSettings;els.worshipToggle.textContent=`Worship: ${s.worshipEnabled?"On":"Off"}`;els.worshipToggle.disabled=!hostSettings;if(!squidSettingsDirty){els.squidTotal.value=s.squid?s.squid.total:0;els.squidPrice.value=s.squid?s.squid.price:0;appliedSquidKey=squidKey();}updateSquidButton(canConfigureSquid,hostSettings&&!!(s.squid&&s.squid.canRestart));if(!settingsDirty){els.smallBlind.value=s.smallBlind;els.bigBlind.value=s.bigBlind;els.actionTimeout.value=s.actionTimeout;els.buyTimeSeconds.value=s.buyTimeSeconds||30;els.peekPriceMin.value=s.peekPriceMin||0;els.peekPriceMax.value=s.peekPriceMax||0;appliedSettingsKey=`${s.smallBlind}|${s.bigBlind}|${s.actionTimeout}|${s.buyTimeSeconds||30}|${s.peekPriceMin||0}|${s.peekPriceMax||0}`;}els.bigBlind.min=els.smallBlind.value;els.startHand.disabled=!s.canStartHand;els.shuffleSeats.disabled=!you.canShuffleSeats;els.addBot.disabled=!(hostSettings&&(s.stage==="lobby"||s.stage==="showdown")&&seated.length<(s.maxSeats||10));els.joinSeat.disabled=!you.canRequestSeat;els.joinSeat.textContent=you.wantsSeat?"Seat Requested":"Join Game Next Hand";els.join.disabled=false;const settingsUnlocked=hostSettings&&(s.stage==="lobby"||s.stage==="showdown");updateSettingsButton(settingsUnlocked);els.smallBlind.disabled=!settingsUnlocked;els.bigBlind.disabled=!settingsUnlocked;els.actionTimeout.disabled=!settingsUnlocked;els.buyTimeSeconds.disabled=!settingsUnlocked;els.peekPriceMin.disabled=!settingsUnlocked;els.peekPriceMax.disabled=!settingsUnlocked;els.rebuyPanel.classList.toggle("hidden",!!you.isSpectator);const showCenterReady=!you.isSpectator&&(s.stage==="lobby"||s.stage==="showdown");const readyTurnsToStart=!!you.readyNextHand;els.readyNext.textContent=readyTurnsToStart?"Start Next Hand":"Ready";els.readyNext.classList.toggle("start-mode",readyTurnsToStart);els.readyNext.disabled=readyTurnsToStart?!s.canStartHand:!you.canReady;els.centerReadyHost.classList.toggle("hidden",!showCenterReady);const minReturn=Math.max(-Math.floor((you.chips||0)/1000),-(you.rebuyUnits||0));els.rebuyUnits.min=minReturn;els.rebuyUnits.placeholder=`Min ${minReturn}`;if(you.refillSubmitted)els.rebuyUnits.value=you.pendingRebuyUnits||0;els.rebuyUnits.disabled=!!you.refillSubmitted;if(Number(els.rebuyUnits.value)<minReturn)els.rebuyUnits.value=minReturn;els.rebuy.textContent=you.refillSubmitted?"Submitted":"Apply Refill";els.rebuy.disabled=!!you.refillSubmitted||!!you.isSpectator;const canChooseBoards=!!you.canChooseBoards,boards=s.communityBoards||[],boardCards=s.communityCards||s.community||[],maxWin=Math.max(0,...(s.winners||[]).map(w=>Number(w.amount)||0)),winnerIds=new Set((s.winners||[]).filter(w=>maxWin>0&&(Number(w.amount)||0)===maxWin).map(w=>w.id)),winnerKey=`${s.handId||""}|${(s.winners||[]).map(w=>`${w.id}:${w.amount}`).join(",")}`;els.board.classList.toggle("multi",boards.length>1);els.board.innerHTML=boards.length>1?boards.map((b,i)=>`<div class="board-row"><span class="board-tag">Board ${i+1}</span>${b.map(c=>cardEl(c)).join("")}</div>`).join(""):(boardCards.length?boardCards:[null,null,null,null,null]).map(c=>cardEl(c)).join("");els.buyTime.classList.toggle("hidden",!you.canAct);els.buyTime.disabled=!you.canBuyTime;els.buyTime.textContent=`Buy Time (${s.buyTimePrice||0})`;els.showEndedHand.classList.toggle("hidden",!you.canShowEndedHand);els.showEndedHand.disabled=!you.canShowEndedHand;els.hand.classList.toggle("folded",!!you.folded);els.hand.innerHTML=((you.cards&&you.cards.length)?you.cards:[null,null]).map(c=>cardEl(c)).join("");els.seats.innerHTML=seated.map((p,i)=>{const pos=seatPositions[i]||[50,50],leftSide=pos[0]<35,rightSide=pos[0]>65,topSide=pos[1]<30,active=p.id===s.currentPlayerId?"active":"",self=p.id===you.id?"self":"",cards=(p.cards||[null,null]).map(c=>cardEl(c,true)).join(""),status=`${p.role||"Online"}${p.folded?" Folded":""}${p.allIn?" All-in":""}${p.connected?"":" Away"}`.trim()||"Online",cardStyle=topSide?"left:34px;top:-44px":leftSide?"left:-56px;top:24px":rightSide?"left:128px;top:24px":"left:34px;top:78px",betStyle=i===0?"left:-42px;top:-18px":radialStyle(pos,96),tokenStyle=i===0?"left:14px;top:-18px":radialStyle(pos,62),color=playerColors[(p.colorIndex||0)%playerColors.length],nameFont=Math.max(9,Math.min(16,Math.floor(150/Math.max(1,String(p.name||"").length))))+"px",bet=p.currentBet>0?`<div class="seat-bet" style="${betStyle}"><span class="bet-label"><span class="chip red"></span>${p.currentBet}</span></div>`:(p.checked?`<div class="seat-bet" style="${betStyle}"><span class="check-label"><span class="chip white"></span>Check</span></div>`:"");const canOfferPeek=!!(p.canPeek||(s.peekEnabled&&you.folded&&!you.isSpectator&&p.id!==you.id&&p.cardCount>0&&!p.privatePeek&&["straddle","preflop","flop","turn","river","board_choice","reveal","optional_show"].includes(s.stage))),peek=canOfferPeek?`<button class="peek-btn" data-peek="${p.id}">wanna see see</button>`:"",botActions=hostSettings&&p.isBot?`<div class="bot-tag-actions">${p.refillSubmitted?`<button class="bot-tag-btn bot-refill-btn" type="button" disabled>Refill set</button>`:`<button class="bot-tag-btn bot-refill-btn" data-refill-bot="${p.id}" type="button">Refill</button>`}${["lobby","showdown"].includes(s.stage)?`<button class="bot-tag-btn bot-remove-btn" data-remove-bot="${p.id}" type="button">Remove</button>`:""}</div>`:"",worship=(s.worshipEnabled&&!you.isSpectator&&p.id!==you.id&&!p.isSpectator)?`<button class="worship-btn" data-worship="${p.id}" title="Worship chips">W</button>`:"",burst=winnerIds.has(p.id)?`<div class="winner-burst"><i></i><i></i><i></i><i></i><i></i><i></i><i></i><i></i></div>`:"",boardChoice=p.id===you.id&&canChooseBoards?`<div class="board-choice-pop">${[1,2,3,4].map(n=>`<button data-board-choice="${n}" ${n>(you.boardChoiceMax||s.boardChoice?.max||1)?"disabled":""}>${n}</button>`).join("")}</div>`:"";let token="";if((p.role||"").includes("Dealer"))token=`<div class="seat-token dealer" style="${tokenStyle}">D</div>`;else if((p.role||"").includes("Small Blind"))token=`<div class="seat-token sb" style="${tokenStyle}">SB</div>`;else if((p.role||"").includes("Big Blind"))token=`<div class="seat-token bb" style="${tokenStyle}">BB</div>`;return `<div class="seat ${self} ${p.folded?"folded":""}" style="left:${pos[0]}%;top:${pos[1]}%" data-player-id="${p.id}"><div class="seat-cards" style="${cardStyle}">${cards}</div>${token}${worship}<div class="badge ${active}" style="border-color:${color}99;--timer-color:${color};--timer-deg:360deg"><div class="pname" style="color:${color};font-size:${nameFont}">${p.name}</div><div class="pchips">${p.chips}</div><div class="pstatus">${status}<br><span class="rebuy-count">Buy-in: ${p.effectiveRebuyUnits??p.rebuyUnits??1}</span>${p.squids?`<br><span class="squid-line">Squid: ${p.squids}</span>`:""}${botActions}${peek}</div>${burst}</div>${boardChoice}${bet}</div>`}).join("");updateBadgeTimers();if((s.winners||[]).length&&winnerKey!==lastWinnerKey){lastWinnerKey=winnerKey;playSound("win");animateWinnerChips((s.winners||[]).filter(w=>Number(w.amount)||0))}else if(!(s.winners||[]).length){lastWinnerKey=""}els.seats.querySelectorAll("[data-board-choice]").forEach(btn=>btn.onclick=e=>{e.stopPropagation();send("choose_boards",{count:Number(btn.dataset.boardChoice||1)})});els.seats.querySelectorAll("[data-peek]").forEach(btn=>btn.onclick=e=>{e.stopPropagation();send("peek_hand",{targetId:btn.dataset.peek})});els.seats.querySelectorAll("[data-remove-bot]").forEach(btn=>btn.onclick=e=>{e.stopPropagation();send("remove_bot",{targetId:btn.dataset.removeBot})});els.seats.querySelectorAll("[data-refill-bot]").forEach(btn=>btn.onclick=e=>{e.stopPropagation();const raw=prompt("Refill how many buy-in units? Use negative to return chips.");if(raw!==null){const units=Number(raw);if(Number.isFinite(units))send("refill_bot",{targetId:btn.dataset.refillBot,units:Math.trunc(units)})}});els.seats.querySelectorAll("[data-worship]").forEach(btn=>btn.onclick=e=>{e.stopPropagation();const amount=Number(prompt("Worship how many chips?")||0);if(Number.isFinite(amount)&&amount>0)send("worship",{targetId:btn.dataset.worship,amount:Math.floor(amount)})});const canAct=!!you.canAct,canStraddle=!!you.canStraddle,canShowdown=!!you.canShowdownAct,canOptionalShow=!!you.canOptionalShow,callNeed=Math.max(0,s.currentBet-(you.currentBet||0));els.prefold.disabled=!(you.canPrefold||you.prefold);els.prefold.innerHTML=you.prefold?"Cancel<br>Prefold":"Prefold";els.prefold.classList.toggle("prefold-on",!!you.prefold);els.prefold.classList.toggle("action-wrap",!!you.prefold);els.fold.disabled=!canAct;els.check.disabled=!canAct||callNeed>0;els.call.disabled=!canAct||callNeed<=0;els.allIn.disabled=!canAct;els.raise.disabled=!canAct;els.raiseAmount.disabled=!canAct;document.querySelectorAll(".quick-bet").forEach(btn=>btn.disabled=!canAct||((you.currentBet||0)+(you.chips||0)<=Math.max((s.currentBet||0)+(s.minRaise||s.bigBlind),s.bigBlind)));els.noStraddle.disabled=!(canStraddle||canShowdown||canOptionalShow);els.straddle.disabled=!(canStraddle||canShowdown||canOptionalShow);const noStraddleLabel=(canShowdown||canOptionalShow)?(canOptionalShow?"Don't<br>Show":"Muck"):"No<br>Straddle";els.noStraddle.innerHTML=noStraddleLabel;els.noStraddle.classList.toggle("action-wrap",noStraddleLabel.includes("<br>"));const canClaimSquidNow=!!(you.canOptionalShow&&s.players.find(p=>p.id===you.id&&p.canClaimSquid));const straddleLabel=(canShowdown||canOptionalShow)?(canOptionalShow?(canClaimSquidNow?"Show +<br>Squid":"Show"):"Show"):(s.straddleAmount?`Straddle ${s.straddleAmount}`:"Straddle");els.straddle.innerHTML=straddleLabel;els.straddle.classList.toggle("action-wrap",straddleLabel.includes("<br>"));els.call.textContent=callNeed?`Call ${callNeed}`:"Call";const minRaiseTo=Math.max((s.currentBet||0)+(s.minRaise||s.bigBlind),s.bigBlind);els.raise.textContent="Raise";els.raiseAmount.min=minRaiseTo;els.raiseAmount.placeholder="Enter for raise";if(canChooseBoards){els.prefold.disabled=true;els.fold.disabled=true;els.check.disabled=true;els.call.disabled=true;els.allIn.disabled=true;els.noStraddle.disabled=true;els.straddle.disabled=true;els.raise.disabled=true;els.raiseAmount.disabled=true;document.querySelectorAll(".quick-bet").forEach(btn=>btn.disabled=true);els.boardChoiceHint.innerHTML=(els.boardChoiceHint.innerHTML?els.boardChoiceHint.innerHTML+"<br>":"")+`Choose boards beside your name tag.<br>Lowest player choice decides.<br>Max ${you.boardChoiceMax||s.boardChoice?.max||1}.`;els.boardChoiceHint.classList.remove("hidden");}}
