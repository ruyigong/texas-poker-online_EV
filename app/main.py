...
class RoomManager:
    ...
    def create_room(self, player_name: str, password: str = "", player_password: str = "", room_name: str = "") -> tuple[Room, Player]:
        ...
        while code in self.rooms:
            code = make_room_code()
        clean_room_name = room_name.strip()[:32] or f"Room {code}"
...
async def list_rooms() -> JSONResponse:
    ...
    for room in manager.rooms.values():
        rooms.append({
            "roomCode": room.code,
            "name": room.name or f"Room {room.code}",
            "stage": room.stage,
            "players": room.seated_count(),
            "maxSeats": MAX_SEATS,
            "hasPassword": bool(room.password_hash),
        })
    rooms.sort(key=lambda item: item["roomCode"])
...
function authName(){return localStorage.getItem("authName")||""}function authPassword(){return localStorage.getItem("authPassword")||""}function setIdentity(){els.name.value=authName();els.playerPassword.value=authPassword();els.lobbyUser.textContent=authName()?`Signed in as ${authName()}`:""}function showLogin(){els.loginPage.classList.remove("hidden");els.lobbyPage.classList.add("hidden");els.gamePage.classList.add("hidden")}function showLobby(){setIdentity();els.loginPage.classList.add("hidden");els.lobbyPage.classList.remove("hidden");els.gamePage.classList.add("hidden");loadRooms()}function showGame(){setIdentity();els.loginPage.classList.add("hidden");els.lobbyPage.classList.add("hidden");els.gamePage.classList.remove("hidden")}async function loadRooms(){try{const res=await fetch("/rooms"),data=await res.json(),rooms=data.rooms||[];els.roomList.innerHTML=rooms.length?rooms.map(r=>`<div class="room-row"><div><div class="room-name">${esc(r.name)} ${r.hasPassword?"[Locked]":""}</div><div class="room-meta">${r.roomCode} | ${String(r.stage).toUpperCase()} | Seats ${r.players}/${r.maxSeats}</div></div><button class="mini" data-room="${r.roomCode}">Join</button></div>`).join(""):`<div class="history-empty">No rooms yet.</div>`;els.roomList.querySelectorAll("button[data-room]").forEach(btn=>btn.onclick=()=>joinLobbyRoom(btn.dataset.room));}catch(err){els.roomList.innerHTML=`<div class="history-empty">Could not load rooms.</div>`}}async function createLobbyRoom(){if(!authName()){showLogin();return}try{els.lobbyNotice.textContent="Creating room...";const res=await fetch("/rooms",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:authName(),playerPassword:authPassword(),roomName:els.newRoomName.value||"",password:els.newRoomPassword.value||""})}),data=await res.json();if(!res.ok)throw new Error(data.error||"Could not create room");roomCode=data.roomCode;playerId=data.playerId;localStorage.setItem("roomCode",roomCode);localStorage.setItem("playerId",playerId);sessionStorage.setItem("playerId",playerId);els.roomCode.value=roomCode;els.roomPassword.value=els.newRoomPassword.value||"";showGame();connect()}catch(err){els.lobbyNotice.textContent=err.message||"Could not create room"}}async function joinLobbyRoom(code){if(!authName()){showLogin();return}try{const res=await fetch(`/rooms/${code}/join`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:authName(),playerPassword:authPassword(),password:els.joinRoomPassword.value||"",playerId:code===roomCode?(playerId||""):""})}),data=await res.json();if(!res.ok)throw new Error(data.error||"Could not join room");roomCode=data.roomCode;playerId=data.playerId;localStorage.setItem("roomCode",roomCode);localStorage.setItem("playerId",playerId);sessionStorage.setItem("playerId",playerId);els.roomCode.value=roomCode;els.roomPassword.value=els.joinRoomPassword.value||"";showGame();connect()}catch(err){els.lobbyNotice.textContent=err.message||"Could not join room"}}
