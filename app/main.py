...
class RoomManager:
    ...
    def create_room(self, player_name: str, password: str = "") -> tuple[Room, Player]:
        code = make_room_code()
    ...
    def join_room(self, code: str, player_name: str, player_id: str | None = None, password: str = "") -> tuple[Room | None, Player | None, str | None]:
...
async def create_room(payload: dict[str, str]) -> JSONResponse:
    ...
    room, player = manager.create_room(player_name, room_password)
...
async def join_room(room_code: str, payload: dict[str, str]) -> JSONResponse:
    ...
    room_password = payload.get("password", "")
    room, player, error = manager.join_room(room_code, player_name, player_id, room_password)
    if error:
...
async function createRoom(){if(creatingRoom)return;creatingRoom=true;try{els.meta.textContent="Creating room...";els.create.disabled=true;const res=await fetch("/rooms",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:els.name.value||"Player",password:els.roomPassword.value||""})}),data=await res.json();if(!res.ok||!data.roomCode||!data.playerId){throw new Error(data.error||data.detail||"Could not create room")}roomCode=data.roomCode;playerId=data.playerId;localStorage.setItem("roomCode",roomCode);localStorage.setItem("playerId",playerId);sessionStorage.setItem("playerId",playerId);els.roomCode.value=roomCode;els.meta.textContent=`Created room ${roomCode}.`;connect()}catch(err){els.notice.textContent=err.message||"Could not create room";els.meta.textContent=`Create failed: ${err.message||"Could not create room"}`;}finally{creatingRoom=false;els.create.disabled=false}}
