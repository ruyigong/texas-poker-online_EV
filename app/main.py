...
const moveStoreKey="pokerPanelPositions:v1";let panelPositions={};try{panelPositions=JSON.parse(localStorage.getItem(moveStoreKey)||"{}")||{}}catch(e){panelPositions={}}let panelZ=140;function savePanelPositions(){localStorage.setItem(moveStoreKey,JSON.stringify(panelPositions))}function clampPanel(el,left,top){const rect=el.getBoundingClientRect(),pad=8,maxLeft=Math.max(pad,window.innerWidth-rect.width-pad),maxTop=Math.max(pad,window.innerHeight-rect.height-pad);return{left:Math.min(Math.max(pad,left),maxLeft),top:Math.min(Math.max(pad,top),maxTop)}}function applyPanelPosition(el,key){const pos=panelPositions[key];if(!pos)return;el.classList.add("drag-floating");el.style.left=`${pos.left}px`;el.style.top=`${pos.top}px`;el.style.width=pos.width?`${pos.width}px`:el.style.width}function ensureMoveHandle(el,key,label="Move"){if(!el)return;el.classList.add("movable-panel");applyPanelPosition(el,key);let handle=el.querySelector(":scope > .move-handle");if(!handle){handle=document.createElement("button");handle.type="button";handle.className="move-handle";handle.title=label;handle.setAttribute("aria-label",label);handle.textContent="+";el.appendChild(handle)}handle.onpointerdown=e=>{if(e.button!==undefined&&e.button!==0)return;e.preventDefault();e.stopPropagation();const rect=el.getBoundingClientRect(),startX=e.clientX,startY=e.clientY,startLeft=rect.left,startTop=rect.top;el.classList.add("drag-floating");el.style.left=`${startLeft}px`;el.style.top=`${startTop}px`;el.style.width=`${rect.width}px`;el.style.zIndex=String(++panelZ);handle.setPointerCapture(e.pointerId);const move=ev=>{const next=clampPanel(el,startLeft+ev.clientX-startX,startTop+ev.clientY-startY);el.style.left=`${next.left}px`;el.style.top=`${next.top}px`};const up=ev=>{handle.releasePointerCapture(ev.pointerId);handle.removeEventListener("pointermove",move);handle.removeEventListener("pointerup",up);handle.removeEventListener("pointercancel",up);const finalRect=el.getBoundingClientRect();const next=clampPanel(el,finalRect.left,finalRect.top);panelPositions[key]={left:Math.round(next.left),top:Math.round(next.top),width:Math.round(finalRect.width)};el.style.left=`${next.left}px`;el.style.top=`${next.top}px`;savePanelPositions()};handle.addEventListener("pointermove",move);handle.addEventListener("pointerup",up);handle.addEventListener("pointercancel",up)};handle.ondblclick=e=>{e.preventDefault();e.stopPropagation();delete panelPositions[key];savePanelPositions();el.classList.remove("drag-floating");el.style.left="";el.style.top="";el.style.width="";el.style.zIndex=""}}function initMovablePanels(){[[els.pokerGod,"pokerGod"],[els.chatPanel,"chat"],[els.historyPanel,"history"],[$("peekPanel"),"peek"],[$("worshipPanel"),"worship"],[document.querySelector(".actions"),"actions"],[els.rebuyPanel,"refill"],[els.scoreboardPanel,"scoreboard"],[els.squidStatus,"squidStatus"]].forEach(([el,key])=>ensureMoveHandle(el,key))}
...
function connect(){if(!roomCode||!playerId)return;if(ws)ws.close();const proto=location.protocol==="https:"?"wss":"ws";ws=new WebSocket(`${proto}://${location.host}/ws/${roomCode}/${playerId}`);ws.onmessage=e=>{const p=JSON.parse(e.data);if(p.type==="state"){render(p);initMovablePanels()}if(p.type==="error")els.notice.textContent=p.message}}
