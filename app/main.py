...
class Room:
    ...
    def toggle_prefold(self, player_id: str) -> str | None:
        ...
        if not self.can_player_prefold(player_id):
            return "Prefold is available only while you are live in a betting round."
        player.prefold = not player.prefold
        if player.prefold and self.can_player_act(player_id):
            self.apply_prefold_actions()
        else:
            self.message = f"{player.name} prefold {'on' if player.prefold else 'off'}."
        return None
