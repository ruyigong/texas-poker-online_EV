...
function send(action,extra={}){if(ws&&ws.readyState===WebSocket.OPEN)ws.send(JSON.stringify({action,...extra}))}function quickBetTo(percent){if(!latest||!latest.state||!latest.you)return;const s=latest.state,you=latest.you,minRaiseTo=Math.max((s.currentBet||0)+(s.minRaise||s.bigBlind),s.bigBlind),betSize=Math.max(s.minRaise||s.bigBlind,Math.round((s.pot||0)*percent)),maxTo=(you.currentBet||0)+(you.chips||0),target=Math.max(minRaiseTo,(s.currentBet||0)+betSize);els.raiseAmount.value=Math.max(0,Math.min(target,maxTo));}
