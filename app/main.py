...
class Room:
    ...
    def earliest_without_squid_after_small_blind(self, without: list[Player]) -> Player | None:
        seated = self.seated_players()
        if not seated or not without:
            return None
        without_ids = {player.id for player in without}
        small_blind_index = (self.dealer_index + 1) % len(seated)
        for offset in range(1, len(seated) + 1):
            candidate = seated[(small_blind_index + offset) % len(seated)]
    ...
    def start_next_squid_series(self, starter: Player | None, total: int, price: int) -> str:
        ...
        self.log_action(starter, "squid", 1, note=f"auto-start next series 1/{self.squid_total}", thinking_time=0.0)
        return f"\nNext squid series started. {starter.name} receives the first squid."
    ...
    def check_squid_end(self) -> str:
        ...
        auto_starter = self.earliest_without_squid_after_small_blind(without)
        self.squid_active = False
        self.squid_total = 0
        self.squid_claimed = 0
        self.squid_price = 0
