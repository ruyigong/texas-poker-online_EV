...
class Room:
    ...
    def add_bot(self, player_id: str) -> str | None:
        ...
        name = f"GTO Bot {self.bot_count() + 1}"
    ...
    def remove_bot(self, player_id: str) -> str | None:
        host_error = self.require_host(player_id)
        if host_error:
            return host_error
        if self.stage not in {"lobby", "showdown"}:
            return "Bots can be removed between hands."
        bot_id = next((pid for pid, player in reversed(list(self.players.items())) if player.is_bot), "")
        if not bot_id:
            return "There are no bots to remove."
        bot_name = self.players[bot_id].name
    ...
    def scoreboard_rows(self) -> list[dict[str, Any]]:
        ...
        for player in self.ordered_players():
            ...
            rows.append({"id": player.id, "name": player.name, "chips": player.chips, "buyIns": effective_units, "buyInChips": buy_in, "net": player.chips - buy_in, "vpip": vpip, "pfr": pfr, "statHands": stat_hands, "isPokerGod": False})
    ...
    try:
        while True:
            ...
            if action == "start_hand":
                ...
                error = room.remove_bot(player_id)
            elif action == "chat":
